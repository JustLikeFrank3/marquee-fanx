...
type Reply={nearby:NearbyData|null;reply:string;results:Results|null;report:Report|null;actions:{name:string;ok:boolean}[]};
type Message={nearby?:NearbyData|null;role:'user'|'assistant';text:string;results?:Results|null;actions?:Reply['actions'];report?:Report|null};
...
export default function Planner({context,onResults}:{context:Search&{budget_usd:number|null;party_size:number};onResults:(results:Results,report:Report|null)=>void}){
  const [configured,setConfigured]=useState<boolean|null>(null),[input,setInput]=useState('');
  const [messages,setMessages]=useState<Message[]>([]),[busy,setBusy]=useState(false),[error,setError]=useState('');
  const [reset,setReset]=useState(true);
  useEffect(()=>{let active=true;fetch('/api/chat/status').then(r=>{if(!r.ok)throw Error();return r.json();}).then(v=>{if(active)setConfigured(v.configured);}).catch(()=>{if(active)setError('Could not check the AI connection. Refresh to retry.');});return()=>{active=false;};},[]);
  async function send(){
